...
use serde::Serialize;
use wormhole::core::CoreAction::{self, *};
use wormhole::core::{ContractUpgrade, GuardianSetChange, SetMessageFee, TransferFees};
use wormhole::VAA;
use wormhole::{Chain, GovAction, GuardianSet};
use wormhole_near::verify_vaa;
...
#[derive(BorshDeserialize, BorshSerialize)]
pub struct Wormhole {
    guardians: LookupMap<u32, GuardianSet>,
...
fn parse_and_verify_vaa(storage: &Wormhole, data: &[u8]) -> VAA {
    let vaa = VAA::from_bytes(data).unwrap_or_else(|_| {
        env::panic_str("Failed to parse VAA");
    });
    ...
    verify_vaa(&vaa, guardian_set, env::block_timestamp() as u32)
        .map_err(|err| {
...
fn vaa_update_guardian_set(
    ...
    current_set.expires = (env::block_timestamp() + storage.guardian_set_expirity) as u32;

    let new_set = GuardianSet {
        addresses: action.new_guardian_set.clone(),
        index: action.new_guardian_set_index,
        expires: 0,
    };
...
impl Wormhole {
    ...
    pub fn boot_wormhole(&mut self, index: u32, addresses: Vec<[u8; 20]>) {
        ...
        self.guardians.insert(&index, &new_set);
        self.guardian_set_index = index;

        env::log_str(&format!("Booting guardian_set_index {}", index));
    }
